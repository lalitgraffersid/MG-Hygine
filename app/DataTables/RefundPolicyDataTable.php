...
class RefundPolicyDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','refund_policy')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','refund_policy')->first();
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("refund_policy.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
     ...
     */
    public function query(RefundPolicy $model)
    {
        //return $model->newQuery();
        $data = RefundPolicy::select();
