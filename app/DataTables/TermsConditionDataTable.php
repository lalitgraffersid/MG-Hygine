...
class TermsConditionDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','terms_and_conditions')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','terms_and_conditions')->first();
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("terms_and_conditions.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
     ...
     */
    public function query(TermsCondition $model)
    {
        //return $model->newQuery();
        $data = TermsCondition::select();
