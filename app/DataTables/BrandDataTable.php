...
class BrandDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $checkStatusAction = Role::where('name_slug','brands')->whereRaw("find_in_set('".$status_action->id."',action_id)")->first();
        $roles = Role::where('name_slug','brands')->first();
        ...
        return datatables()
            ...
            ->editColumn('status', function($row) use ($checkStatusAction,$checkStatusPermission) {
                if (!empty($checkStatusAction) && (!empty($checkStatusPermission) || Auth::user()->user_type == 'admin')) {
                    if($row->status == '1'){
                        return '<a href="'.route("brands.status",$row->id).'" class="btn btn-success btn-sm" onclick="return confirm("Are you sure want to change status?")">Active</a>';
                    }
                    if($row->status == '0'){
                        return '<a href="'.route("brands.status",$row->id).'" class="btn btn-danger btn-sm" onclick="return confirm("Are you sure want to change status?")">Inactive</a>';
                    }
                }else{
                    if($row->status == '1'){
                ...
                foreach ($action_ids as $key => $action_id) {
                    ...
                    if (!empty($checkPermission) || Auth::user()->user_type == 'admin') {
                        if ($action->action_slug == 'edit' || $action->action_slug == 'delete' || $action->action_slug == 'view' || $action->action_slug == 'password') {
                            $btn .= '<a href="'.route("brands.$action->action_slug",$row->id).'" class="btn btn-'.$action->class.' btn-sm" data-placement="top" data-original-title="'.$action->action_title.'"><i class="'.$action->icon.'"></i>'.$action->action_title.'</a>&nbsp;';
                        }
                    }
                }
                return $btn;
