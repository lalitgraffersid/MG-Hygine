...
class ProductDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->editColumn('image', function($row) {
                $image = ProductImage::where('product_id',$row->id)->first();
                if (!empty($image)) {
                    $url = url('/public/admin/clip-one/assets/products/thumbnail').'/'.$image->image;
                }else{
    ...
    public function query(Product $model)
    {
        ...
        $data = Product::join('brands','products.brand_id','=','brands.id')
                    ...
                    ->select('products.*','brands.name as brand','categories.title as category');
    ...
    protected function getColumns()
    {
        return [
            ...
            'category' => ['name' => 'categories.title'],
            'title' => ['sWidth' => '200px'],
